...
export function Flashcard({ word, index, total, onResult }: FlashcardProps) {
  ...
  return (
    <div className="flex w-full max-w-sm flex-col items-center gap-5">
      ...
      >
        <div className="preserve-3d grid w-full transition-transform duration-500" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
          <div className="backface-hidden col-start-1 row-start-1 flex min-h-[280px] flex-col items-center justify-center gap-5 rounded-2xl border-2 bg-card p-8 shadow-[0_6px_0_rgba(0,0,0,0.07)] dark:shadow-[0_6px_0_rgba(0,0,0,0.3)]">
            <p className="text-[11px] font-extrabold uppercase tracking-widest" style={{ color: "var(--app-primary)" }}>What does this mean?</p>
            ...
            <AudioRow />
            <p className="text-xs text-muted-foreground">{revealed ? "Tap to flip" : "Tap to reveal meaning"}</p>
          </div>
          ...
          <div
            ...
            style={{ transform: "rotateY(180deg)", borderColor: "color-mix(in srgb, var(--app-primary) 30%, transparent)", backgroundColor: "color-mix(in srgb, var(--app-primary) 5%, var(--card))" }}
          >
            <div className="flex flex-1 flex-col items-center justify-center gap-4 w-full">
              ...
              <div className="flex flex-col items-center gap-2">
                <span className="rounded-full border-2 px-3 py-0.5 text-xs font-bold" style={{ borderColor: "color-mix(in srgb, var(--app-primary) 40%, transparent)", color: "var(--app-primary)" }}>{word.word}</span>
            ...
            {examples.length > 0 && (
              <div className="w-full space-y-1.5 border-t pt-4" style={{ borderColor: "color-mix(in srgb, var(--app-primary) 20%, transparent)" }}>
                <p className="text-[10px] font-extrabold uppercase tracking-widest" style={{ color: "color-mix(in srgb, var(--app-primary) 70%, transparent)" }}>Examples</p>
                {examples.slice(0, 2).map((ex, i) => (
                  <p key={i} className="text-xs leading-relaxed text-foreground/70 italic">&ldquo;{highlightExample(ex, word.word)}&rdquo;</p>
                ))}
              </div>
            )}
          </div>
      ...
      {!revealed ? (
        <button onClick={handleFlip} className="btn-duo w-full flex items-center justify-center gap-2 rounded-xl border-2 py-3.5 text-sm font-extrabold transition-colors" style={{ borderColor: "var(--app-primary)", backgroundColor: "color-mix(in srgb, var(--app-primary) 10%, transparent)", color: "var(--app-primary)" }}>
          Reveal answer <ChevronRight className="h-4 w-4" />
        </button>
      ) : (
        <div className="w-full space-y-2">
          <button onClick={handleFlip} className="w-full flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors py-1">
            <RotateCcw className="h-3 w-3" />{flipped ? "See word" : "See meaning"}
          </button>
          <div className="flex gap-3">
            <button onClick={() => onResult(false)} className="btn-duo flex-1 rounded-xl border-2 border-[#ff4b4b] bg-[#ff4b4b]/10 py-3.5 text-sm font-extrabold text-[#ff4b4b] shadow-[0_4px_0_rgba(255,75,75,0.3)] hover:bg-[#ff4b4b]/20 transition-colors">✗ Wrong</button>
            <button onClick={() => onResult(true)} className="btn-duo flex-1 rounded-xl border-2 py-3.5 text-sm font-extrabold transition-colors" style={{ borderColor: "var(--app-primary)", backgroundColor: "color-mix(in srgb, var(--app-primary) 10%, transparent)", color: "var(--app-primary)", boxShadow: "0 4px 0 color-mix(in srgb, var(--app-primary) 30%, transparent)" }}>✓ Correct</button>
          </div>
          <p className="text-center text-[10px] text-muted-foreground/50">[Space] flip · [←/j] wrong · [→/l] correct</p>
        </div>
      )}
    </div>
  );
