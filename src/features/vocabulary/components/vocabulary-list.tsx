...
import { WordDialog } from "@/features/vocabulary/components/word-dialog";
import { DeleteWordButton } from "@/features/vocabulary/components/delete-word-button";
import { speakWord } from "@/lib/speech";
import { refetchWordAudio } from "@/features/vocabulary/actions/vocabulary.actions";
...
function masteryAccent(w: VocabularyWord) {
...
function masteryBadge(w: VocabularyWord): { label: string; bg: string; color: string } | null {
...
function Pagination({ page, totalPages, q }: { page: number; totalPages: number; q: string }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1 pt-2">
      {page > 1 ? (
        <Link href={pageUrl(page - 1, q)} className={`${btnBase} ${btnIdle}`}><ChevronLeft className="h-4 w-4" /></Link>
      ) : (
      ...
      {pages.map((p, i) =>
        p === "..." ? (
          ...
          <Link key={p} href={pageUrl(p, q)} className={`${btnBase} text-sm ${p === page ? btnActive : btnIdle}`}>{p}</Link>
        )
      )}
      {page < totalPages ? (
        <Link href={pageUrl(page + 1, q)} className={`${btnBase} ${btnIdle}`}><ChevronRight className="h-4 w-4" /></Link>
      ) : (
...
export function VocabularyList({ words, total, page, totalPages, q }: Props) {
  ...
  function playAudio(word: VocabularyWord, e: React.MouseEvent) {
    e.stopPropagation();
  ...
  return (
    <div className="space-y-3">
      <div className="relative">
        ...
        />
        {inputValue && (
          <button onClick={() => setInputValue("")} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs font-bold text-muted-foreground hover:text-foreground">✕</button>
        )}
      </div>
        ...
        <>
          <div className="space-y-2">
            {words.map((w) => {
              ...
              return (
                <div key={w.id} className="group flex items-stretch rounded-2xl border-2 bg-card overflow-hidden transition-all hover:shadow-sm" style={{ borderLeftColor: masteryAccent(w), borderLeftWidth: "4px" }}>
                  <div className="flex flex-1 items-center gap-3 px-4 py-3 min-w-0">
                    <div className="flex-1 min-w-0 overflow-hidden">
                      <div className="flex items-baseline gap-1.5 min-w-0">
                        ...
                        {w.ipa && <span className="hidden sm:inline font-mono text-xs text-muted-foreground truncate">{w.ipa}</span>}
                      </div>
                      <p className="mt-0.5 text-sm text-muted-foreground truncate">{w.meaning}</p>
                    </div>
                    <div className="flex shrink-0 items-center gap-0.5">
                      {badge && <span className="hidden sm:block rounded-full px-2 py-0.5 text-[11px] font-extrabold mr-1" style={{ backgroundColor: badge.bg, color: badge.color }}>{badge.label}</span>}
                      <button onClick={(e) => playAudio(w, e)} className="flex h-8 w-8 items-center justify-center rounded-xl text-[#1cb0f6] hover:bg-[#1cb0f6]/15 transition-colors" title="Play audio"><Volume2 className="h-3.5 w-3.5" /></button>
                      <button onClick={(e) => handleRefresh(w.id, w.word, e)} disabled={refreshingId === w.id} className="hidden sm:flex h-8 w-8 items-center justify-center rounded-xl text-muted-foreground hover:bg-muted hover:text-foreground transition-colors disabled:opacity-40" title="Refresh IPA & audio"><RefreshCw className={`h-3.5 w-3.5 ${refreshingId === w.id ? "animate-spin" : ""}`} /></button>
                      <WordDialog mode="edit" word={{ id: w.id, word: w.word, meaning: w.meaning, examples: w.userExamples }} trigger={<button className="flex h-8 w-8 items-center justify-center rounded-xl text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"><Pencil className="h-3.5 w-3.5" /></button>} />
