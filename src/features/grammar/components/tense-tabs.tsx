...
import { tenses, groups, type TenseGroup } from "@/features/grammar/constants/tenses.constants";
    ...
    <div className={cn("rounded-2xl border-2 bg-card overflow-hidden transition-all", open && "shadow-md")}>
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-muted/30 transition-colors">
        ...
        <div className="flex items-center gap-3 shrink-0 ml-3">
          <code className="hidden md:block text-xs font-mono px-2.5 py-1 rounded-lg bg-muted" style={{ color: tense.color }}>{tense.structure}</code>
      ...
      {open && (
        <div className="border-t border-border px-5 pb-5 space-y-5 pt-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {[{ label: "(+) Positive", value: tense.structure }, { label: "(–) Negative", value: tense.negative }, { label: "(?) Question", value: tense.question }].map(({ label, value }) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <ul className="space-y-1.5">
                {tense.usage.map((u) => (
                  <li key={u} className="flex items-start gap-2 text-sm">
                    <span className="mt-1 shrink-0 h-1.5 w-1.5 rounded-full" style={{ backgroundColor: tense.color }} />{u}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {tense.signals.map((s) => (
                  <span key={s} className="rounded-full px-2.5 py-0.5 text-xs font-bold" style={{ backgroundColor: `${tense.color}18`, color: tense.color }}>{s}</span>
                ))}
              </div>
            </div>
          </div>
...
export function TenseTabs() {
  ...
  const groupColors: Record<TenseGroup, string> = { present: "var(--app-primary)", past: "#ff4b4b", future: "#ffc800" };

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-2 rounded-2xl border-2 bg-muted/40 p-1.5">
        {groups.map(({ key, label, emoji }) => (
          <button key={key} onClick={() => setActive(key)} className={cn("flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-extrabold transition-all", active === key ? "bg-card shadow-sm" : "text-muted-foreground hover:text-foreground")} style={active === key ? { color: groupColors[key] } : {}}>
            <span>{emoji}</span>
      ...
      <div className="flex items-center gap-2 px-1">
        ...
        <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">{filtered.length} tenses</p>
      </div>
      <div className="space-y-2">
        {filtered.map((t) => <TenseCard key={t.id} tense={t} />)}
      </div>
    </div>
  );
}
