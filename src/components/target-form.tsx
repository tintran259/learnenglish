...
import { setDailyTarget } from "@/actions/stats";
    ...
    <form ref={formRef} action={handleAction} className="flex gap-2">
      <div className="flex flex-1 items-center rounded-lg border-2 bg-background transition-colors focus-within:border-[#58cc02] overflow-hidden">
        <input
          ...
          max={500}
          required
          className="flex-1 px-3 py-2.5 text-sm font-bold bg-transparent outline-none w-16"
        />
        <span className="pr-3 text-sm text-muted-foreground font-medium whitespace-nowrap">
          words / day
        </span>
      </div>
      <button
