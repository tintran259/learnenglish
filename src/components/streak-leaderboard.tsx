...
function RankBadge({ rank }: { rank: number }) {
  ...
  return (
    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-muted text-[11px] font-extrabold text-muted-foreground">
      {rank}
    </span>
  );
}
  ...
  return (
    <div className="h-1.5 w-16 rounded-full bg-muted overflow-hidden">
      <div
        ...
        style={{ width: `${pct}%`, backgroundColor: "var(--app-primary)" }}
      />
    </div>
  );
}
...
export function StreakLeaderboard({ entries, myRank, totalUsers, myStreak }: Props) {
  ...
  // Show current user separately at bottom if not in visible range
  ...
  return (
    <div className="space-y-1">
      ...
      {/* Rows */}
      {visible.map((entry) => (
        <Row key={entry.userId} entry={entry} maxStreak={maxStreak} />
      ))}

      {/* Gap + current user if outside visible */}
        ...
        <>
          ...
          {showMyRowBelow && (
            <div className="flex items-center gap-3 rounded-xl bg-[color-mix(in_srgb,var(--app-primary)_8%,transparent)] border-2 border-[color:var(--app-primary)]/20 px-3 py-2.5">
              ...
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-muted text-xs font-extrabold shrink-0">
                Y
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-extrabold" style={{ color: "var(--app-primary)" }}>You</p>
              </div>
      ...
      {entries.length > PREVIEW && (
        <button
          ...
          className="mt-1 w-full flex items-center justify-center gap-1 rounded-xl py-2 text-xs font-bold text-muted-foreground hover:bg-muted transition-colors"
        >
          {expanded ? (
            <><ChevronUp className="h-3.5 w-3.5" /> Show less</>
          ) : (
            <><ChevronDown className="h-3.5 w-3.5" /> Show {entries.length - PREVIEW} more</>
          )}
        </button>
      )}
      ...
      {entries.length === 0 && (
        <p className="text-center text-sm text-muted-foreground py-4">
          No one has a streak yet. Be the first! 🔥
        </p>
      )}
    </div>
  );
  ...
  return (
    <div
      className={cn(
        ...
        entry.isCurrentUser
          ? "border-2 border-[color:var(--app-primary)]/25 bg-[color-mix(in_srgb,var(--app-primary)_8%,transparent)]"
    ...
    >
        ...
        <div
          ...
          style={{
            backgroundColor: entry.isCurrentUser
              ...
              : "var(--muted)",
            color: entry.isCurrentUser ? "var(--app-primary)" : "var(--muted-foreground)",
          }}
        >
      ...
      {/* Name */}
      <p
        className={cn("flex-1 min-w-0 text-sm font-extrabold truncate", entry.isCurrentUser && "text-[color:var(--app-primary)]")}
      >
        {displayName(entry.name, entry.isCurrentUser)}
      </p>
