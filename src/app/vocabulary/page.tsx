...
import { getVocabularyPage, getStats } from "@/features/vocabulary/actions/vocabulary.actions";
    ...
    <main className="mx-auto max-w-3xl px-4 py-8 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Your personal vocabulary collection</p>
        </div>
        <WordDialog
          mode="add"
          trigger={
          ...
          <div className="grid grid-cols-3 divide-x divide-border">
            {[
              ...
              <div key={label} className="flex flex-col items-center gap-1 px-3 py-4 text-center">
                ...
                <p className="text-xl font-black leading-none tabular-nums mt-0.5" style={{ color }}>{value}</p>
      ...
      {stats.total === 0 ? (
        ...
        <VocabularyList words={words} total={total} page={page} totalPages={totalPages} q={q} />
      )}
    </main>
  );
}
